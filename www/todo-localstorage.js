...
todoStorage.add = function(label){
    ...
    if(this.hasItem(label)) { return false; }
...
todoStorage.remove = function(label){
    ...
    this.collection.forEach(function(item, i){
       if(item.label === label) { this.collection.splice(i, 1); } 
    });
...
todoStorage.toggleStatus = function(label){
    ...
    this.collection.forEach(function(item, i){
       item.status = item.status === 'complete' ? 'uncomplete' : 'complete';
    });
...
todoStorage.filter = function(status){
    ...
    if(status === 'all'){ return this.collection; }
